...
import os
import sys
# required for medicc issues with large samples
...
if 'absl.logging' in sys.modules:
  # necessary to stop annoying custom gradient warnings from model save/load
  import absl.logging
...
def run_hmm(observed_counts, rpc_prior_value, zero_prior, alpha_max_clip, alpha_prior_value, alpha_zero_prior_value, alpha_prior_scale,
  ...
  # Inference robust to changes in ploidy for small rpc values
  if rpc_prior_value < 5:
    trainable_rpc = tf.Variable(rpc_prior_value, name="rpc", dtype=tf.float32,
                                constraint=lambda x: \
                                     tfp.math.clip_by_value_preserve_gradient(x,
                                                   clip_value_min=0.95*rpc_prior_value,
                                                   clip_value_max=1.05*rpc_prior_value))
  elif rpc_prior_value < 10:
    trainable_rpc = tf.Variable(rpc_prior_value, name="rpc", dtype=tf.float32,
                                constraint=lambda x: \
                                     tfp.math.clip_by_value_preserve_gradient(x,
                                                   clip_value_min=0.9*rpc_prior_value,
                                                   clip_value_max=1.1*rpc_prior_value))
  else:
    trainable_rpc = tf.Variable(rpc_prior_value, name="rpc", dtype=tf.float32,
                                constraint=lambda x: \
                                     tfp.math.clip_by_value_preserve_gradient(x,
                                                   clip_value_min=0.75*rpc_prior_value,
                                                   clip_value_max=1.25*rpc_prior_value))
...
def interface_hmm(data, rpc, alpha,
  ...
  n_oscillations =  np.count_nonzero(np.diff(state_vector))
  magnitude_oscillations =  np.sum(np.abs(np.diff(state_vector)))
  sd_oscillations = np.std(np.diff(state_vector))
  optimum["n_oscillations"] = n_oscillations
  optimum["sd_oscillations"] = sd_oscillations
  optimum["magnitude_oscillations"] = magnitude_oscillations
...
def interface_compute_pvalues(data, name, hmm_path, h0, h1, start, end, n_samples=10000, len_cutoff=20):
  import logging
