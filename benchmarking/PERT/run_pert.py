...
cn = cn[cn['chr'] != "X"]
n_train_samples = 200
# bad both with unbalanced and balanced datasets
n_test_samples_g1 = 85
n_test_samples_s = 285
...
test_g1_set = random.sample([cellnames[i] for i,_ in enumerate(cellcycle) if cellcycle[i] == "G1" and cellnames[i] not in train_g1_set], n_test_samples_g1)
test_s_set = random.sample([cellnames[i] for i,_ in enumerate(cellcycle) if cellcycle[i] == "S"], n_test_samples_s)
...
df.to_csv('membership_100.tsv', sep='\t', index=False)

#NOTE: not possible to run max_iter > 540 for step 2, at 0.01 learning rate -> program crashes with NaN everywhere
scrt = scRT(temp_cn, temp_cn_g1, input_col='copy', clone_col='clone_id', assign_col='copy', rt_prior_col=None,
            cn_state_col='state', gc_col='gc', cn_prior_method='g1_clones', max_iter=400,
...
import pickle
with open("/home/schnei01/scdna_replication_tools/result_100.pkl", "wb") as output_file:
    pickle.dump(result, output_file)

confusion_matrix = pd.crosstab(result['true_cellcycle'], result['PERT_phase'], rownames=['Actual'], colnames=['Predicted'])
print(confusion_matrix)
# NOTE to subtract the original training samples to fairly compare only on test set
