...
/* Original author: Rebecca Killick<r.killick@lancs.ac.u
 * https://github.com/rkillick/changepoint/
 * minor modifications Michael P Schneider */
...
void PELT(cost_func, sumstat, n, m, pen, cptsout, error, shape, minorder, optimalorder, maxorder, minseglen, tol, lastchangelike, lastchangecpts, numchangecpts, MBIC, alpha, rpc)
  char **cost_func;
  ...
  double *alpha; // global value for alpha
  double *rpc; // global value for rpc
  {
...
void meanvar_negbin_ml();
void meanvar_negbin_mm();
void meanvar_negbin_bcmm();
void meanvar_negbin_cml();
void meanvar_negbin_alpha();
void meanvar_negbin();
void state_space();
void state_space_penalized();
/* void mll_reg(); */
//void mll_ar();

if (strcmp(*cost_func,"meanvar.negbin_ml")==0){
costfunction = &meanvar_negbin_ml;
}
else if (strcmp(*cost_func,"meanvar.negbin_mm")==0){
costfunction = &meanvar_negbin_mm;
}
else if (strcmp(*cost_func,"meanvar.negbin_bcmm")==0){
costfunction = &meanvar_negbin_bcmm;
}
else if (strcmp(*cost_func,"meanvar.negbin_cml")==0){
costfunction = &meanvar_negbin_cml;
}
else if (strcmp(*cost_func,"meanvar.negbin_alpha")==0){
costfunction = &meanvar_negbin_alpha;
}
else if (strcmp(*cost_func,"meanvar.negbin")==0){
costfunction = &meanvar_negbin;
}
else if (strcmp(*cost_func,"state_space")==0){
costfunction = &state_space;
}
else if (strcmp(*cost_func,"state_space_penalized")==0){
costfunction = &state_space_penalized;
}
 ...
 //Evaluate cost for second minseglen
  start = 0;
	for(j = *minseglen; j < (2 * *minseglen); j++){
      costfunction(sumstat, &size, &np1, &p, minorder, optimalorder, maxorder, &start, &j, &segcost, tol, error, shape, MBIC, alpha, rpc);
  ...
  for(tstar=2 * (*minseglen); tstar < np1;tstar++){
   ...
   if ((lastchangelike[tstar]) == 0){
  		for(i=0;i<(nchecklist);i++){
				...
				costfunction(sumstat, &size, &np1, &p, minorder, optimalorder, maxorder, &start, &tstar, &segcost, tol, error, shape, MBIC, alpha, rpc);
