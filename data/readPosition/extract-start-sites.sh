...
# Later modifications by: Austin Reed
...
# sort bam file
samtools sort -o ${input_bam}.sorted ${input_bam}

# mark duplicates
picard MarkDuplicates I=${input_bam}.sorted O=${input_bam}.deduplicated M=${input_bam}.duplicate_metrics.txt REMOVE_DUPLICATES=true VALIDATION_STRINGENCY=LENIENT
...
#
if [ $(samtools view -c -f 1 ${input_bam}.sorted) = "0" ]; then
    # single-end reads
